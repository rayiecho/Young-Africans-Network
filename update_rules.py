...
# Read existing rules and add studio rules before the last closing brace
existing_rules = open(sa_path.replace('serviceAccount.json', 'current_rules.txt'), 'r').read() if False else None

new_studio_rules = '''
    // ── YAN STUDIO COLLECTIONS ──
...
# Full rules with studio additions inserted before last closing braces
full_rules = '''rules_version = '2';
...
resp = requests.post(
    ...
    headers=headers,
    json={'source': {'files': [{'name': 'firestore.rules', 'content': full_rules}]}}
)
...
if resp.status_code == 200:
    ...
    rel = requests.patch(
        ...
        headers=headers,
        json={'name': f'projects/{project_id}/releases/cloud.firestore', 'rulesetName': ruleset_name}
    )
    print(f"Release: {rel.status_code}")
    if rel.status_code == 200:
        print("✅ Firestore rules updated — studio collections added!")
    else:
        print(rel.text)
else:
    print(resp.text)
