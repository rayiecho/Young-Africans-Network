...
# Find startRecording function and replace it
start = None
end = None
for i, line in enumerate(lines):
    if 'async function startRecording()' in line:
        start = i
    if start and 'async function stopRecording' in line:
        end = i
        break
...
if start and end:
  ...
  // Request tab audio capture — this captures Web Speech API audio
  let tabStream = null;
  try {
    ...
    addLog('info', '🎤 Tab audio capture enabled — voice will be in video!');
  } catch(e) {
    addLog('info', '⚠ Audio capture skipped — video will be silent. To enable voice, click Allow when browser asks.');
  }
  ...
  function speakScene(scene) {
    ...
    utt.rate = speed2; utt.pitch = 1; utt.volume = 1;
...
'''
    ...
    with open('yan-studio.html', 'w') as f:
        f.writelines(lines)
    print(f"✅ startRecording replaced (lines {start+1} to {end+1})")
else:
